...
func displayArticle(article Article) {
	...
	if doc != nil {
		doc.Find("div[class=entry-content]>p,div[class=entry-content]>ol,div[class=entry-content]>ul,div[class=entry-content]>pre,div[class=entry-content] .code-container").Each(func(i int, s *goquery.Selection) {
			fmt.Println(s.Text())
		})
	}
}
